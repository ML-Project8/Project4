...
clean_file <- function(data_file_name, agency_subset = c(), dat_header, agency_trans_table) {
    ...
    # if Age is unspecified, use median age for agency and education
    df$Age <- with(df, ave(df$Age, df$Agency, df$Education, FUN = function(x) replace(x, is.na(x), levels(df$Age)[median(as.integer(x), na.rm = TRUE)])))
